...
const ReservationCalendar = ({
  ...
  return (
    <div className="relative w-121">
      ...
      {isMonthMenuOpen && (
        <div
          className="absolute left-0 top-10 z-20 flex w-30 flex-col overflow-hidden rounded-xl border bg-white py-1 shadow-[0_0_0.5rem_rgba(0,0,0,0.08)]"
        ...
        >
          {monthOptions.map(option => (
            <button
              key={option.key}
              type="button"
              className="flex h-8 w-full items-center justify-center whitespace-nowrap font-['Pretendard'] text-[0.9375rem] font-medium leading-[1.3] transition-colors hover:bg-[#F7F7F7]"
              style={{
                color: option.active
